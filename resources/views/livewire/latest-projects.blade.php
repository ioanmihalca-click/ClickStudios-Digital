    ...
    <div class="container px-4 mx-auto">
        ...
        <div class="grid grid-cols-1 gap-8 md:grid-cols-3">
            @foreach ($projects as $project)
                <div
                    class="relative overflow-hidden transition-all duration-300 border group bg-gray-800/50 rounded-xl border-gray-700/50 hover:border-emerald-500/50">
                    <!-- Project Image with Overlay -->
                    <div class="relative overflow-hidden">
                        <img src="{{ $project['image'] }}" alt="{{ $project['title'] }}"
                            class="object-cover w-full h-48 transition-transform duration-300 group-hover:scale-110">
                        <div
                            class="absolute inset-0 transition-opacity duration-300 opacity-0 bg-gradient-to-t from-gray-900 via-gray-900/40 to-transparent group-hover:opacity-100">
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex flex-wrap gap-2 mb-6">
                            @foreach ($project['technologies'] as $tech)
                                <span
                                    class="px-3 py-1 text-xs font-medium rounded-full bg-emerald-500/10 text-emerald-400">
                                    {{ $tech }}
                                </span>
                            @endforeach
                        </div>
